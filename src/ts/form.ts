...
//import { iceProductsObject, cakeProductsObject, Product } from "./fetches"
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const iceList: string[] = ["1", "2", "3", "4"]
  const cakeList: string[] = ["1", "2"]

  const htmlIceList: string = ""
  const htmlCakeList: string = ""
  /*
      let fulliceList: Product[] | void = []
      let fullcakeList: Product[] | void = []
    
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      fulliceList = iceProductsObject
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      fullcakeList = cakeProductsObject
      
      fulliceList.forEach((product) => {
        iceList.push(product.id.toString())
        htmlIceList += `<option value="${product.id.toString()}" > ${
          product.name
        }</option>`
      })
      fullcakeList.forEach((product) => {
        cakeList.push(product.id.toString())
        htmlCakeList += `<option value="${product.id.toString()}" > ${
          product.name
        }</option>`
      })
    */
  ...
  const productManager: ProductManager = new ProductManager(
    ...
    <option disabled value="">Ice</option>
               <option value="1" >Vanille</option>
               <option value="2" >Mokka</option>  
  ...
  const cakeManager: ProductManager = new ProductManager(
    ...
    `
               ...
               <option disabled value="">Cake</option>
                   <option value="1" >Vanille Cake</option>
                   <option value="2" >Mokka Cake</option>
    ...
    /*error
            ...
            if (hiddenField.value == "icecream") {
              ...
              productElements.forEach(function (product1, index) {
                ...
                form.elements[productSelect.name].value,
                  form.elements[quantityInput.name].value
              })
            }
  ...
  function addRules(index: number) {
    if (index == 0) {
      validator.addValidator({
        ...
        field: form.elements["fname"] as HTMLInputElement,
      })
      validator.addValidator({
        name: "lname",
        method: (field) => field.value.length >= 2,
        message: "Name must be at least 2 characters long.",
        field: form.elements["lname"] as HTMLInputElement,
      })
      ...
      if (hiddenField.value == "icecream") {
        ...
        if (productElements.length < 1) {
          ...
          productElements.forEach(function (product1, index) {
            ...
            if (!iceList.includes(productSelect.value)) {
              validator.addValidator({
                name: hiddenField.name,
                method: () => false,
      ...
      if (hiddenField.value == "icecake") {
        ...
        if (productElements.length < 1) {
          ...
          productElements.forEach(function (product1, index) {
            ...
            if (!cakeList.includes(productSelect.value)) {
              validator.addValidator({
                name: hiddenField.name,
                method: () => false,
                message: "Please choose a valid product",
